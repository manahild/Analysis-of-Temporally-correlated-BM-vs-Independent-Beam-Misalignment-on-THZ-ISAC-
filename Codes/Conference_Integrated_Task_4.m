...
%  AND THE FULL CORRELATED-MISALIGNMENT SENSING SIMULATOR
%  ------------------------------------------------------------------------
...
%  complete-chain validation) merged in.
%
%  WHAT CHANGED VS. THE TWO SOURCE FILES (Tasks2/3/5/6 master + standalone
%  Task 4 script)
%  ----------------------------------------------------------------------
%  1) Task 4's duplicate copies of setup_system_params, build_radar_
%     reference, generate_received_signal_fast, range_doppler_fast,
%     beam_gain_general, mysinc2, beam_matrix_A, make_beam_case, tf_str,
%     save_figure_t2 were DELETED -- this file still has exactly one copy
%     of each, now shared by Tasks 1, 2, 3, 4, and the Task 5/6 loop.
%  2) Task 4's Stage A / Stage A2 / Stage B functions (run_stage_A,
%     run_stage_A2_diagnostics, run_stage_B) and their unique helpers
%     (g_observation, jacobian_fd, gauss_newton_solve, lookup_estimate,
%     directional_radius, target_bins, probe_gate_power,
%     calibrate_detection_threshold, measure_probe_pair) are added
%     VERBATIM (formulas/logic unchanged) as new local functions of this
%     file, wrapped by one new function run_task4_validation() that owns
%     the folder setup / parameter bundling the standalone script used to
%     do at script level.
%  3) Task 4's placeholder "wref = 0.20 m (ASSUMPTION)" is replaced by the
%     SAME wref2 = w_mo_SFref that Tasks 2 and 3 already use in this
%     master script -- Task 4's own local functions (run_stage_A/A2/B)
%     already took wref as a parameter, so no internal changes to their
%     bodies were needed; only the value passed in at the call site
%     changed, exactly mirroring how Task 2/3's placeholders were
%     resolved when they were first wired into this master file.
%  4) run_task4_validation() is called once, immediately after
%     run_task3_validation(), using wref2 and the same 5-beam-case list
%     already defined for Tasks 2/3/5/6. Its outputs (Figures_T4/,
%     Results_T4/Task4_StageA_table.csv, Task4_StageA2_diagnostics_
%     table.csv, Task4_StageB_table.csv) are produced alongside the other
%     tasks' outputs, with no changes to the Task 5/6 sensing loop, radar
%     equation, range-Doppler processing, or Section 10/11 stationary-
%     covariance check.
%  5) fdstep4 and the Stage-B reduced-combo-set parameters (beamsB4,
%     alphaListB4, Ntrials4, Mcal4, Nfa4, Pfa_target4, jitterStd4) are
%     declared once in new Section 2.6, mirroring how Task 3's sweep
%     constants live in Section 2.5.
%
%  VALIDATION NOTE (from the pre-merge standalone Task 4 console run):
%  Direct and Gauss-Newton recover eps to ~1e-13 to 1e-17 m for every
%  beam. EKF-1-step is exact for the 3 quadratic (Gaussian-family) beams
%  but blows up at cold start (eps_pred=0) for FlatTop/Sidelobed. Stage A2
%  confirms this is a linearization-DISTANCE artifact, not a Jacobian-
%  conditioning bug: J is well-conditioned at cold start (rcond ~0.22-
%  0.35) and the EKF error falls smoothly to 0 as the linearization point
%  marches toward the truth. Stage B confirms the practical consequence:
%  EKF-1-step from cold start is unusable for FlatTop (RMSE 244-669 mm),
%  while Lookup stays reliable (RMSE 5.6-12.5 mm). Net: EKF-1-step is only
%  safe as a TRACKING filter (after the first, near-truth prediction),
%  never as a cold-start estimator for non-quadratic beams -- Lookup or
%  GN should be used instead at initialization for those beams.
%
%  Everything else about the radar waveform, range-Doppler processor,
%  AR(1) recursion, metric definitions, and Task 2/3/5/6 loop structure is
%  UNCHANGED from the Task 2/3/5/6 master script.
% =========================================================================
...
for f = {figDir2,resDir2,figDir3,resDir3,figDir4,resDir4,figDir,resDir,datDir}
...
%     pointing-estimation section, AND the Task 5/6 sensing loop below,
%     via make_beam_case(name, wref).
%% ------------------------------------------------------------------
...
%% ------------------------------------------------------------------
%  10. SAVE DATA (now includes the beam-case dimension + Task 4 params)
%% ------------------------------------------------------------------
...
%  estimation, AND the Task 5/6 sensing loop above)
%% ========================================================================
