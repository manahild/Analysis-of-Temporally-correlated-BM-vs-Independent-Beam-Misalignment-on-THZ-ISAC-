%% ========================================================================
%  TASKS 2, 5 & 6 -- GENERALIZED BEAM-MISALIGNMENT LIBRARY INTEGRATED INTO
%  THE FULL CORRELATED-MISALIGNMENT SENSING SIMULATOR
%  ------------------------------------------------------------------------
%  This is the standalone "Task 2" beam-map library script wired into the
%  "Task 5 & 6" performance-fixed sensing simulator, per the brief:
%
%     "How much sensing performance is lost when a tracker assumes a
%      circular Gaussian beam while the actual monostatic THz beam is
%      elliptical, rotated, flat-topped, or sidelobed..."
%
%  WHAT CHANGED VS. THE TWO SOURCE SCRIPTS
%  ----------------------------------------------------------------------
%  1) The Task 2 local functions (beam_gain_general, beam_matrix_A,
%     beam_quadratic_matrix, beam_gain_numerical, mysinc2, cut_widths,
%     find_crossing, tf_str, fmt_or_na, save_figure_t2) are copied in
%     verbatim (unchanged formulas/logic) as local functions of this file.
%  2) The Task 2 validation section (6 checks + 5 required beam-case
%     figures/width table) is wrapped into run_task2_validation() and is
%     still run exactly once, but now with wref = the ACTUAL w_mo computed
%     by compute_beam_width() for the SF regime, instead of the
%     placeholder wref = 0.20 used when Task 2 was standalone. This
%     resolves the "ASSUMPTION" TODO left in the original Task 2 file.
%  3) The Task 5/6 main sensing loop no longer calls the old circular-only
%     beam_gain(eps, beam_parameters) [Eq.12 only]. It now loops over the
%     SAME 5 required beam cases from Task 2 (circular Gaussian,
%     elliptical Gaussian, rotated elliptical Gaussian, p=4 flat-top,
%     sidelobed sinc^2) and evaluates h(eps) with beam_gain_general().
%     wx=wy=wref for the circular case and wref=w_mo of the regime under
%     test, exactly matching Task 2's convention ("wref = wmo for the
%     original circular beam and keep this value when comparing modified
%     profiles").
%  4) Per the brief -- "Reuse exactly the same trajectory when comparing
%     beam models" -- the AR(1)/i.i.d./aligned pointing trajectories
%     eps_k are now generated ONCE per regime (not once per beam case) and
%     the same four trajectories are reused across all 5 beam cases; only
%     h(eps) changes between beam cases, not eps itself.
%  5) results / T6 / figures / saved .mat now carry an extra "beam case"
%     dimension: results.(regime).(beamTag).(waveform).(scenarioTag)...
%     Everything else (radar equation, range-Doppler processing, metrics,
%     Section 10 stationary-covariance check) is unchanged.
%  6) The legacy circular-only beam_gain() [Eq.12] is kept at the bottom
%     for reference/backward compatibility but is no longer called by the
%     main loop -- beam_gain_general() supersedes it.
%
%  Nothing about the radar waveform, range-Doppler processor, AR(1)
%  recursion, or metric definitions was changed.
% =========================================================================
...
for f = {figDir2,resDir2,figDir,resDir,datDir}
...
%     validation section AND the Task 5/6 sensing loop below, via
...
%  wref is now the ACTUAL w_mo of the SF regime (computed below), replacing
...
%% ========================================================================
%  LOCAL FUNCTIONS -- Task 2 beam-map library (copied in verbatim from the
%  standalone Task 2 script; formulas/logic unchanged)
%% ========================================================================
